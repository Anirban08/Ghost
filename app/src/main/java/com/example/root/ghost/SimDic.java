...
public class SimDic implements GhostDictionary {
    // singleton class, so we do not need to reload the dictionary every time
    // the activity is recreated
    private static SimDic sInstance;

    private ArrayList<String> words;
    private Random random = new Random();

    // function to access the singleton
    public static SimDic get(InputStream wordListStream) throws IOException {
        // if the dictionary is uninitialized
        if(sInstance == null){
            // initialize the dictionary
            sInstance = new SimDic(wordListStream);
        }
    ...
    // constructor
    public SimDic(InputStream wordListStream) throws IOException {
